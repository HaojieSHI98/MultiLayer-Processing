...
//                    cout<<"start querying..."<<endl;
                    ...
                    if (is_thresholded)
                        ...
                        kNNs = handleQueryThreshold(copy_id, _threadid, method_name, k, _taskid.first, mems.dist, mems.visited,
                                                    mems.q,
                                                    dijkstra_object_map, globalThreadVar[copy_id]->ran_threshold, query_id);
                    else
                        kNNs = handleQuery(copy_id, _threadid, method_name, k, _taskid.first, mems.dist, mems.visited, mems.q,
                                           dijkstra_object_map, globalThreadVar[copy_id]->ran_threshold, query_id);
...
//                    cout<<"aggregate task added!"<<endl;
                ...
                if (_taskid.second == INSERT) {
                    handleInsert(copy_id, _threadid, method_name, k, _taskid.first, dijkstra_object_map, mems);

                }
                if (_taskid.second == DELETE) {
                    handleDelete(copy_id, _threadid, method_name, k, _taskid.first, dijkstra_object_map, mems);
...
//                cout<<"here"<<endl;
                ...
                for (int j = start_q_id; j < num_queues_selected + start_q_id; j++) {
